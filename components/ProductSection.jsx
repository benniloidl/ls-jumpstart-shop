            ...
            <div className="bg-red-100x flex flex-col space-y-8 p-20 border-2 rounded-md shadow-md">
                ...
                { Array.isArray(products)
                    ? products.map((product, productIndex) => {
                        ...
                        const sapCommerceReference = _.get(fields, "sapCommerceReference");
                        if (!sapCommerceReference) return "";
                        
                        if (sapCommerceReference.type === "product")
                            return <ProductCardComponent productIndex={ productIndex }
                                                         key={ productId }
                                                         id={ productId }
                                                         fields={ fields }/>
                        else if (sapCommerceReference.type === "category")
                            return <CategoryCardComponent productIndex={ productIndex }
                                                          key={ productId }
                                                          id={ productId }
                                                          fields={ fields }/>
                    })
